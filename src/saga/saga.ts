...
function* validateUser(action: any) {
    ...
    try {
        const response: any = yield call(twitterService.validateUser, { username, password });
...
function* changePassword(action: any) {
    ...
    try {
        const response: any = yield call(twitterService.updatePassword, { username, password, dob });
        if (response?.status === 200) {
...
function* createUser(action: any) {
    ...
    try {
        const response: any = yield call(twitterService.addUser, { name, username: phone, dob, password, profileImage });
